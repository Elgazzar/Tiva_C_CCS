...
void Switch_init()
{
    ...
    GPIOPadConfigSet(GPIO_PORTF_BASE, GPIO_PIN_4, GPIO_STRENGTH_4MA,
                     GPIO_PIN_TYPE_STD_WPU); /* Set Pin as Pull-up */
...
void switch2_init()
{
    ...
    GPIOPinTypeGPIOInput(GPIO_PORTD_BASE, GPIO_PIN_0); /* Set pin 4 in PortF to be Input */

    GPIOPadConfigSet(GPIO_PORTD_BASE, GPIO_PIN_0, GPIO_STRENGTH_4MA,
                         GPIO_PIN_TYPE_STD_WPU); /* Set Pin as Pull-up */
}
...
uint8_t Switch2_Read(void)
{
    uint8_t Button2_Read = GPIOPinRead(GPIO_PORTD_BASE, GPIO_PIN_0);
